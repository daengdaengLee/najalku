# Descheduler: TopologySpreadConstraint와 플러그인 비교
...
Part A에서 다룬 핵심 개념을 짚고 넘어간다.

**Descheduler의 역할**: kube-scheduler는 파드 배치를 일회성으로 결정한다. Descheduler는 이미 실행 중인 파드를 주기적으로 검사해, 현재 배치가 최적이 아니면 해당 파드를 **evict**한다. evict된 파드는 kube-scheduler가 더 적합한 노드로 재배치한다.
...
**Handle → Evictor → DefaultEvictor 연결**: 플러그인은 `Handle`을 통해서만 클러스터에 접근한다. `handle.Evictor()`가 돌려주는 `evictorImpl`은 초기화 시 `DefaultEvictor`의 Filter 함수들을 등록해두어, 플러그인이 eviction을 요청할 때 자동으로 DefaultEvictor의 판별 로직이 실행된다.
...
evict 대상 판별은 두 단계로 나뉜다: `Filter`(저비용, 후보 선정)로 먼저 좁힌 뒤, `PreEvictionFilter`(고비용, 최종 확인 — 대표적으로 NodeFit)를 통과한 파드만 실제 eviction된다.

**RemoveDuplicates**: `BalancePlugin` 구현체. 같은 `ownerKey`(namespace + kind + name + imagesHash)를 가진 파드가 한 노드에 몰린 경우를 탐지하고, `upperAvg = ceil(전체 파드 수 / targetNode 수)` 기준으로 초과분을 즉시 evict한다.

## 2. RemovePodsViolatingTopologySpreadConstraint

`RemovePodsViolatingTopologySpreadConstraint`는 `RemoveDuplicates`와 마찬가지로 `BalancePlugin`을 구현한다. 클러스터 전체의 파드 분포를 보고 재분배가 필요한 파드를 evict한다.
...
> **참고**: 소스 코드의 주석(L419, L464)은 "낮은 우선순위를 먼저 evict"하겠다는 의도를 기술하고 있다. 그러나 실제 구현에서 `!comparePodsByPriority`는 낮은 우선순위를 슬라이스 앞쪽에, 높은 우선순위를 뒤쪽에 배치한다. eviction이 뒤쪽부터 가져가므로 결과적으로 **높은 우선순위가 먼저 evict**된다. 코드 주석과 구현이 불일치하는 사례다.
