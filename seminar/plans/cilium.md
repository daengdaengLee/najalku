...
#### 유닛 2 — eBPF 훅 부착 지점 상세 (개념)

- 핵심 질문: Cilium 이 사용하는 세 훅(XDP / tc / socket)은 각각 커널 어디에서 어떤 컨텍스트에 접근해 무엇을 처리하는가?
- 범위
  - XDP: NIC 드라이버 직후, sk_buff 생성 전, raw 패킷, XDP_DROP/PASS/TX/REDIRECT
  - tc ingress/egress: qdisc·class·filter 구조, sk_buff 단위, TC_ACT_OK/SHOT/REDIRECT
  - socket: cgroup 부착, connect()/sendmsg() 훅, 패킷 NAT 가 아닌 destination rewrite 모델
  - 세 훅의 위치 비교 다이어그램 (NIC → 애플리케이션 경로 위에 표기)
  - Cilium 의 활용 예시 한 줄씩 (XDP 부하분산 / tc 보안 정책 집행 / socket-LB) — 깊이 다루지 않음
- 범위 외
  - 다른 eBPF 훅 (kprobe / uprobe / tracepoint / cgroup-기타 / lsm) — 유닛 1 의 한 줄 언급 유지
  - L7 리다이렉트·Envoy 통합 (선택적 확장)
  - Cilium 의 컴포넌트 토폴로지 (다음 유닛)
- 완결 기준: 패킷 1 개가 NIC → 애플리케이션까지 가는 흐름 위에 세 훅의 부착 위치와 각 훅의 처리 권한 차이를 설명 가능
- 코드 분석 없음

#### 유닛 3 — Cilium 아키텍처 개요 (개념)
...
#### 유닛 7 — 코드: Agent watch → BPF map 갱신 (코드)
...
#### 유닛 10 — NetworkPolicy 코드: 정책 컴파일러 (코드)
...
#### 유닛 11 — NetworkPolicy 코드: BPF policy lookup (코드)
...
- NodePort / LoadBalancer / IPv6 LB: 유닛 4 의 ClusterIP 한정 후속
- bpf_lxc.c (tc hook) 패킷 경로: 유닛 8 의 socket-LB 와 별도 흐름
- CIDR / FQDN NetworkPolicy: 유닛 9 의 표준 정책 후속
...
- [x] 유닛 3: Cilium 아키텍처 (0009-cilium-architecture.md)
- [ ] 유닛 4: Cilium Service LB (ClusterIP)
- [ ] 유닛 5: 실습 설치+검증 (kind)
- [ ] 유닛 6: 실습 BPF map 조회 (kind, CLI)
- [ ] 유닛 7: 코드 Agent → BPF map (Service add 단일 경로)
- [ ] 유닛 8: 코드 BPF C 프로그램 (bpf_sock.c)
- [ ] 유닛 9: NetworkPolicy 개념 (identity 의미 + 정책 결정)
- [ ] 유닛 10: NetworkPolicy 코드 — 정책 컴파일러 (Go)
- [ ] 유닛 11: NetworkPolicy 코드 — BPF policy lookup (C)
- [ ] 유닛 12: Hubble 내부 구조
