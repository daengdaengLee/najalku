...
kube-scheduler는 파드가 새로 생성될 때 노드를 선택합니다. 이 결정은 **일회성**입니다. 한번 배치된 파드는 노드가 더 적합한 상태로 바뀌어도 자동으로 이동하지 않습니다.
...
새 파드는 D, E에 배치되지만, A/B/C에 이미 있던 파드들은 그대로입니다. 시간이 지나면 같은 ReplicaSet 파드들이 특정 노드에 쏠리거나, 노드 간 부하 불균형이 고착됩니다.

kube-scheduler는 이 문제를 해결하지 않습니다. **새 파드를 어디에 놓을지**만 결정할 뿐, **기존 파드를 옮기는 역할**은 없기 때문입니다.

**Descheduler**는 이 간극을 채웁니다. 이미 실행 중인 파드를 주기적으로 검사해, 현재 배치가 최적이 아니라고 판단하면 해당 파드를 **evict(퇴출)** 합니다. Evict된 파드는 kube-scheduler에 의해 다시 스케줄링되고, 이번엔 더 적합한 노드에 배치됩니다.

> **중요**: Descheduler는 파드를 직접 이동시키지 않습니다. evict 후 재스케줄링은 kube-scheduler가 담당합니다.

## 2. 전체 동작 흐름

#### 핵심 용어 정리

코드를 따라가기 전에 세 가지 용어의 관계를 먼저 잡고 가겠습니다.
...
Policy는 설정의 최상위 단위이고, Profile은 플러그인 조합에 이름을 붙인 묶음입니다. 동일한 플러그인이더라도 서로 다른 Profile에서 다른 설정으로 사용할 수 있습니다.
...
`runProfiles()`는 각 Profile의 `RunDeschedulePlugins`, `RunBalancePlugins`를 순서대로 호출합니다.
...
Deschedule을 먼저 실행해 "명백한 문제(규칙 위반)" 파드를 제거하고, 그 후 Balance가 남은 파드들을 재분배합니다. 두 단계를 분리함으로써 Balance가 이미 정리된 상태를 기준으로 균형을 계산할 수 있습니다.

#### 실행 모드

Descheduler는 세 가지 방식으로 배포할 수 있습니다.
...
CronJob과 Deployment는 주기적으로 실행한다는 점은 같지만, 이전 실행이 끝나지 않은 경우의 동작이 다릅니다.
...
Deployment 방식은 단일 프로세스가 루프를 돌기 때문에 애초에 중복 실행이 불가능합니다. `wait.NonSlidingUntil`은 이전 실행이 반드시 끝난 후 남은 interval 시간을 기다리고, 초과했다면 끝나는 즉시 다음 실행을 시작합니다.

interval이 `0`이면 한 번 실행 후 context를 cancel해서 루프를 종료합니다 (`pkg/descheduler/descheduler.go:546-548`).

#### SharedInformerFactory — 왜 캐시를 쓰는가?

플러그인들은 매 루프마다 클러스터의 노드/파드 목록을 조회합니다. 매번 API 서버에 직접 요청하면 부하가 크기 때문에, client-go의 **Informer**를 사용해 로컬 캐시에서 읽습니다.

```
API 서버 --Watch--> Informer (로컬 캐시) --List--> 플러그인
```

`WaitForCacheSync()`는 첫 루프가 시작되기 전, 캐시가 완전히 채워질 때까지 블로킹합니다.

다음 섹션에서는 플러그인이 어떤 인터페이스로 구성되어 있는지, eviction 판단이 어떤 구조로 이루어지는지 살펴봅니다.

## 3. Plugin 시스템

호출 스택에서 `SetupPlugins()`이 플러그인을 레지스트리에 등록하고, `NewProfile()`이 각 플러그인을 인스턴스화하는 것을 봤습니다. **Profile**은 플러그인 조합에 이름을 붙인 단위로, 하나의 Policy에 여러 Profile을 정의할 수 있고 각 Profile은 독립적인 플러그인 집합을 가집니다.
...
모든 플러그인은 `pkg/framework/types/types.go`에 정의된 인터페이스를 구현합니다.
...
플러그인은 생성 시 `Handle`을 주입받습니다. 클러스터에 접근하는 **모든 수단**이 Handle을 통해서만 제공됩니다.
...
`Handle.Evictor()`의 리턴 타입은 `Evictor` 인터페이스입니다. `EvictorPlugin`과는 별개로, `Filter`/`PreEvictionFilter`에 더해 실제 eviction을 요청하는 `Evict()`까지 포함합니다.
...
런타임에 `Handle.Evictor()`가 돌려주는 실제 값은 `evictorImpl`(`pkg/framework/profile/profile.go`)입니다. `NewProfile()` 초기화 시 `EvictorPlugin`(DefaultEvictor)의 Filter 함수들을 `evictorImpl`에 미리 등록해둡니다.
...
이 덕분에 플러그인이 `handle.Evictor().Filter(pod)`를 호출하면 자동으로 DefaultEvictor의 판별 로직이 실행됩니다.
...
`DefaultEvictor`는 `EvictorPlugin`의 구현체입니다. Deschedule/Balance 플러그인이 "이 파드를 evict하고 싶다"고 요청하면, DefaultEvictor가 "evict해도 되는 파드인가"를 판단합니다.
...
**Filter**: 아래 조건 중 하나라도 해당하면 evict 대상에서 제외합니다.
...
단, 파드에 `descheduler.alpha.kubernetes.io/evict` 어노테이션이 있으면 위 규칙을 전부 무시하고 evict 대상으로 취급합니다.

**PreEvictionFilter**: 실제 eviction 직전에 수행하는 최종 확인입니다. `Filter`로 후보를 먼저 좁힌 뒤, 추려진 파드에만 실행하기 때문에 비용이 큰 검사를 여기에 둡니다. 대표적으로 **NodeFit** 옵션이 여기서 동작합니다. 해당 파드가 다른 노드에 스케줄될 수 있는지 확인하고, 갈 곳이 없으면 evict하지 않습니다.
...
같은 ReplicaSet이나 Deployment에 속한 파드들이 특정 노드에 몰려 있는 상황을 해결합니다.
...
kube-scheduler가 처음 배치할 당시엔 최적이었더라도, 이후 노드가 추가되거나 다른 파드가 evict되면서 불균형이 생길 수 있습니다. `RemoveDuplicates`는 이런 상태를 감지하고 초과분을 evict해 재스케줄링을 유도합니다.

### 핵심 용어 정리

코드에 등장하는 용어를 먼저 잡고 가겠습니다.

| 용어 | 정의 |
|---|---|
| **`podOwner`** | 파드의 "소유자 + 이미지" 조합을 담는 구조체. `namespace`, `kind`, `name`, `imagesHash` 4개 필드로 구성됩니다. |
| **`ownerKey`** | `podOwner` 구조체 값 자체를 맵의 키로 사용할 때의 명칭. "이 파드는 어떤 owner 그룹에 속하는가"를 식별하는 단위입니다. |
| **`imagesHash`** | 파드의 컨테이너 이미지 목록을 정렬 후 `"#"`으로 연결한 문자열. 예: `"nginx:1.25#sidecar:v2"`. 같은 owner라도 이미지가 다르면 다른 그룹으로 취급합니다. |
| **`podContainerKeys`** | 파드 하나를 `"namespace/kind/name/image"` 형태의 문자열 목록으로 표현한 것 (정렬됨). 이 목록이 같은 파드 두 개가 한 노드에 있으면 **중복**으로 판단합니다. |
| **`duplicateKeysMap`** | 노드 안에서 중복을 탐지하는 임시 맵. 노드마다 초기화되므로 **같은 노드 내** 중복 탐지에만 쓰입니다. |
| **`ownerKeyOccurence`** | ownerKey별 전체 파드 수. **모든 노드**를 합산한 값으로, `upperAvg` 계산에 사용됩니다. |
| **`duplicatePods`** | `ownerKey → 노드이름 → []*v1.Pod` 구조. 각 노드에서 중복으로 판별된 파드 목록. 각 노드의 **첫 번째 파드(기준 파드)는 포함되지 않습니다.** |
| **`upperAvg`** | 균등 분배 시 노드당 허용할 최대 파드 수. `ceil(전체 파드 수 / targetNode 수)` 로 계산합니다. |
| **`targetNodes`** | 파드가 실제로 스케줄 가능한 노드 목록. toleration, nodeSelector, nodeAffinity를 기준으로 필터링합니다. |
...
노드를 순회하면서 각 파드의 `ownerKey`와 `podContainerKeys`를 만듭니다. (`L110-L162`)
...
`podContainerKeys`가 준비되면, 같은 노드에서 이미 같은 키를 봤는지 `duplicateKeysMap`으로 확인합니다. (`L165-L193`)
...
`duplicateKeysMap`은 노드마다 초기화됩니다. 따라서 **한 노드 안에서 같은 ownerKey를 가진 파드가 2개 이상 있는 경우**만 `duplicatePods`에 기록됩니다. 노드 간 파드 수 불균형은 Phase 2의 `upperAvg`로 처리합니다.
...
evict된 파드는 kube-scheduler에 의해 Node C에 재배치됩니다.

#### getTargetNodes — 실제로 갈 수 있는 노드만 계산

> `L236-L291`

`upperAvg`를 계산할 때 단순히 전체 노드 수를 쓰면 안 됩니다. 파드의 toleration, nodeSelector, nodeAffinity 때문에 일부 노드엔 스케줄이 불가능할 수 있기 때문입니다.
...
파드가 실제로 재배치될 수 없는 노드는 분모에서 제외되어 `upperAvg`가 더 정확하게 계산됩니다.

## 5. 다음 시간 예고

다음 세미나(Part B)에서는 두 번째 플러그인인 `RemovePodsViolatingTopologySpreadConstraint`를 다룹니다. TSC의 `maxSkew` 위반을 two-pointer 알고리즘으로 해소하는 방식을 코드 레벨로 살펴보고, `RemoveDuplicates`와의 설계 차이를 비교합니다.
